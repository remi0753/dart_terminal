...
copyrighted standards. Retrieval date is 2026-09-06.
...
Families are `ecma48`, `dec`, and `xterm`. Selector kinds are `c0`, `c1`,
`esc`, `csi`, `osc`, `dcs`, `sos`, `pm`, `apc`, and `mode`. Mode names include
`private-` when their selector uses the DEC private marker. The typed selector,
not the human-readable syntax, is the uniqueness key.
