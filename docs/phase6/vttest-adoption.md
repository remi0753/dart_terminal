...
| VT-27 | VT510/VT520 DECRQSS, DSR, and screen-mode additions | `MANUAL` | The seven-byte DECRQSS SGR request is now automated with exact xterm and semantic Kitty differential evidence. DECNCSM/DECATC and advanced reports remain evidence-driven. |
| VT-28 | xterm version, mode, and status-string reports | `MANUAL` | DECRQSS SGR, both XTVERSION forms, and XTWINOPS 14/18 are byte/split-tested and exercised through both product runtimes. Raw product-specific DECRQSS serialization remains visible; other status strings stay manual and evidence-driven. |
...
| VT-30 | xterm mouse and DEC locator reports | `MANUAL` | X10/1000/1002/1003 with UTF-8/SGR/URXVT and SGR pixel routing is automated, including native backing-scale geometry and Shift-local ownership. Focus reporting is also automated. Mode 1001 highlight tracking and DEC locator remain explicit unsupported without an observed enable/handshake requirement. |
