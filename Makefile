...
help:
	...
	@echo "  make runtime-window-interaction-integration  Verify one window input owner and responder restoration in both modes"
...
durable-file-dart-test:
	...
	@cd $(PROJECT_ROOT)/packages/dart_durable_file_macos && \
		$(DART) format --output=none --set-exit-if-changed hook lib test
	@cd $(PROJECT_ROOT)/packages/dart_durable_file_macos && $(DART) analyze
	@cd $(PROJECT_ROOT)/packages/dart_durable_file_macos && \
		$(DART) run test/run_tests.dart
...
terminal-note-disabled-input-benchmark-build: dependencies
	@mkdir -p $(PRODUCT_PARSER_BENCHMARK_DIR)
	@cd $(PROJECT_ROOT) && $(DART) compile exe \
		tool/terminal_note_disabled_input_benchmark.dart \
		-o $(TERMINAL_NOTE_DISABLED_INPUT_BENCHMARK)
...
developer-jit-window-interaction: developer-jit-build
	@cd $(PROJECT_ROOT) && $(INTEGRATION_TOOL) --mode=developer-jit \
		--suite=window-interaction $(DEVELOPER_JIT_BUNDLE)
...
developer-jit-note-s1: developer-jit-build
	@cd $(PROJECT_ROOT) && $(INTEGRATION_TOOL) --mode=developer-jit \
...
release-aot-window-interaction: release-aot-build
	@cd $(PROJECT_ROOT) && $(INTEGRATION_TOOL) --mode=release-aot \
		--suite=window-interaction $(RELEASE_AOT_BUNDLE)
...
release-aot-note-s1: release-aot-build
	@cd $(PROJECT_ROOT) && $(INTEGRATION_TOOL) --mode=release-aot \
...
runtime-window-interaction-integration: \
	developer-jit-window-interaction release-aot-window-interaction
...
runtime-note-s1-integration: developer-jit-note-s1 release-aot-note-s1

contextual-memory-s1-acceptance: terminal-notes-capability-audit \
