...
| UI-09 | settings UI、command palette、terminal inspector | P1 | 8/10 | `G:macos/Sources/Features/Settings/`, `Command Palette/`, InspectorView | Shift-Command-Pのbounded searchable native command palette、shared `application.open-settings` action、Command-comma menuに加え、全36 optionを含むmodal root-document editorを実装。`/`検索、同一syntax表示のNORMAL/INSERT、caret連動detail/rail、invalid draftの下線・非永続化、valid atomic save/shared reload、single-window/focus/全handle cleanupをM1 Developer JIT/Release AOTで受け入れ済み。terminal/parser inspectorはPhase 10 |
