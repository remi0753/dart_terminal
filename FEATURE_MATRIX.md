...
| TXT-05 | cell metrics、baseline、underline/strike、1x/2x/scale/zoom の pixel alignment | P0 | 4 | `G:src/font/Metrics.zig`, `G:src/renderer/generic.zig` | cell/decorations metrics、macOS標準13ptのzero-config既定値、1x/2x atlas domain、scale/font changeのatomic resource rebuildを完了。zoom入力は後続UI |
