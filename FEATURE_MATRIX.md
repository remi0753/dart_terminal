...
| QA-02 | xterm/Ghostty/Kitty black-box differential と real-app matrix。bug を最小 byte regression に還元 | P1 | 6 | `G:src/terminal/` tests and VT C examples | 260件のECMA-48/DEC/xterm/iTerm selector・mode baselineと85件の製品実装照合、versioned observation/driver contract、Dart backend、bounded failure分類を完了。実comparator adapter/captureとreal-app matrixは後続task |
