...
| TXT-03 | CoreText discovery、ordered fallback、Apple Color Emoji、symbol font | P0 | 4 | `G:src/font/discovery.zig`, `G:src/font/DeferredFace.zig`, `G:src/font/face/coretext.zig` | generation-owned catalog、CJK/color emoji fallback、全run face identity/copy、zero-configのmacOS system monospace選択を完了 |
...
| TXT-05 | cell metrics、baseline、underline/strike、1x/2x/scale/zoom の pixel alignment | P0 | 4 | `G:src/font/Metrics.zig`, `G:src/renderer/generic.zig` | cell/decorations metrics、macOS system monospace 14ptのzero-config既定値、top-down CoreText glyph raster、1x/2x atlas domain、scale/font changeのatomic resource rebuildを完了。zoom入力は後続UI |
...
| IN-05 | char/word/line multi-click selection、drag、autoscroll | P0 | 5 | `G:src/terminal/Selection.zig`, `SelectionGesture.zig`, SurfaceView | stable anchor gesture、forward/reverse drag、history viewport、bounded edge autoscroll、Metal overlay、実AppKit両mode受け入れを完了 |
...
| IN-07 | standard clipboard、bracketed paste、newline normalization | P0 | 5 | `G:src/input/paste.zig`, `G:src/terminal/paste.zig`, NSPasteboard helpers | bounded plain-text Copy/Paste menu、DEC 2004、CRLF/CR/LF正規化、危険control置換、one-frame streamingを完了 |
