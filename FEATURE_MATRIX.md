...
| CAP-04 | xterm-256color 互換 terminfo と SSH fallback | P0/P1 | 6/8 | `G:src/terminfo/`, `G:src/cli/ssh.zig` | ncurses 6.6.20251230のbase projectionを固定し、未実装・危険capabilityをcancelしたsource、3,761-byte compiled entry、両runtime bundle resource、byte/semantic freshness gateを完了。製品はheader/name/layout検証後にlocal `TERMINFO`へ接続し、欠落・破損時とremote PTYではprivate pathを除外して標準名へfallback。character-set/XTGETTCAP closureは順序付き後続 |
