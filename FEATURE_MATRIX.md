...
| UI-09 | settings UI、command palette、terminal inspector | P1 | 8/10 | `G:macos/Sources/Features/Settings/`, `Command Palette/`, InspectorView | Shift-Command-Pのbounded searchable native command paletteに加え、shared `application.open-settings` action、Command-comma menu、schema-derived effective-config検索/選択/detail、直近diagnostic、shared reload、focus復元、single-window/handle cleanupを実装。menu/palette/shared action、検索、rejected/applied reload、pane/PTY identity、focus/handle cleanupをM1 Developer JIT/Release AOTで受け入れ済み。terminal/parser inspectorはPhase 10 |
...
| CFG-07 | deprecated migration warning、effective config/action docs の schema generation | P1 | 8 | `G:src/config/`, CLI config commands | physical key、pane/application action、default binding、native予約shortcut、全36 optionのcanonical formatter、deprecated theme migration warning、`--help`/`--show-config`、configuration/CLI Markdown、native Settings inspectorをshared schema/catalogから生成して通常freshness gate化。worker/AppKit ownerを作らないeffective-config early exitと、invalid startup/reload diagnostic、Settings内canonical provenance/policyをM1 Developer JIT/Release AOTで受け入れ済み |
