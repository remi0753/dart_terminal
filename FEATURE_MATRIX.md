...
| TXT-05 | cell metrics、baseline、underline/strike、1x/2x/scale/zoom の pixel alignment | P0 | 4/8 | `G:src/font/Metrics.zig`, `G:src/renderer/generic.zig` | cell/decorations metrics、terminal本文とSettings主編集面で共有するmacOS system monospace regular / 14ptのzero-config既定値、Retina scaleを反映したLatin/CJK/color emojiの1x/2x logical ink parity、top-down CoreText glyph raster、mixed ASCII/CJK cluster原点とselection overlayの1x/2x pixel alignment、scale/font changeのatomic resource rebuildを完了。zoom入力は後続UI |
