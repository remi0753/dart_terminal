...
| CAP-07 | XTGETTCAP、DECRQSS、window/size report | P1 | 6 | `G:src/terminal/dcs.zig`, `G:src/terminal/size_report.zig` | XTGETTCAPはbounded hex/list検証後、監査DBにないcapabilityを明示的 unavailable と応答し、`Ms`/OSC 52を先行広告しない。完全なDECRQSS SGRは現在属性/ANSI・256・direct色を63 byte以下のxterm互換形式で返し、xterm exact/Kitty semantic differentialを完了。window/size reportは次のtask |
