...
| UI-09 | settings UI、command palette、terminal inspector | P1 | 8/10 | `G:macos/Sources/Features/Settings/`, `Command Palette/`, InspectorView | Shift-Command-Pのbounded searchable native command palette、shared `application.open-settings` action、Command-comma menuに加え、key入力前から全36 optionを表示するmodal root-document editorを実装。`/`検索、同一syntax表示のNORMAL/INSERT、commented optionの全行disabled色、modeをまたぐ全幅current-line背景、NORMAL/検索selectionのnative viewport追従、caret連動detail/rail、invalid draftの下線・非永続化、valid atomic save/shared reload、single-window/focus/全handle cleanupをM1 Developer JIT/Release AOTで受け入れ済み。terminal/parser inspectorはPhase 10 |
...
| CFG-04 | reload と live/new-session/restart policy を option ごとに宣言 | P0 | 8 | `G:src/config/Config.zig`, macOS Config | 全36 optionにlive/new-session policyを宣言。明示Reload Configuration action、同一起動入力でのsingle-flight再解決、error候補（利用不能なfile値を含む）の全体拒否とlast-known-good保持、既存paneのkeybind/Option live適用、後続pane/windowのnew-session適用を実装し、invalid→corrected reloadとresource identity/cleanupをM1 Developer JIT/Release AOTで受け入れ済み |
