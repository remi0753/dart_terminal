...
| UI-08 | AppleScript application→windows→tabs→terminals と App Intents | P1 | 10 | `G:macos/Sources/Features/AppleScript/`, `App Intents/` | AppleScriptはstandard window→tab→terminalのstable typed ID、title/local cwd、selected/focused関係をcontent-freeなbounded native cacheで公開し、new window/tab、4方向split、input、focus、terminal/tab/window closeを既存hierarchy/paste/close policyへ接続。Quick Terminalは除外し、live disable/re-enable、stale/wrong-kind/busy/timeout/shutdownのfail-closedとexactly-once completionを実装。canonical sdef/plist、実PTY exact input、10 command、4 session/全owner回収をM1 Developer JIT/Release AOTで受け入れ、外部TCC確認は[manual checklist](docs/phase10/applescript-manual-acceptance.md)へ分離。App Intents/Shortcutsは次のPhase 10項目で未実装 |
| UI-09 | settings UI、command palette、terminal inspector | P1 | 8/10 | `G:macos/Sources/Features/Settings/`, `Command Palette/`, InspectorView | Shift-Command-Pのbounded searchable native command palette、shared `application.open-settings` action、Command-comma menuに加え、key入力前から全45 optionを表示するmodal root-document editorを実装。`/`検索、同一syntax表示のNORMAL/INSERT、commented optionの全行disabled色、modeをまたぐ全幅current-line背景、NORMAL/検索selectionのnative viewport追従、caret連動detail/rail、invalid draftの下線・非永続化、valid atomic save/shared reload、single-window/focus/全handle cleanupをM1 Developer JIT/Release AOTで受け入れ済み。Quick Terminal shortcutの登録状態・競合理由・旧登録保持、およびSecure Keyboard Entryのmode/ownership/自動取得/indicator設定をbounded statusとして表示する。terminal/parser inspectorはPhase 10 |
...
| CFG-04 | reload と live/new-session/restart policy を option ごとに宣言 | P0 | 8 | `G:src/config/Config.zig`, macOS Config | 全45 optionにlive/new-session policyを宣言。明示Reload Configuration action、同一起動入力でのsingle-flight再解決、error候補（利用不能なfile値を含む）の全体拒否とlast-known-good保持、既存paneのkeybind/Option、Quick Terminalのshortcut/screen/animation/autohide、Secure Keyboard Entryの自動取得/indicator、AppleScriptのcache/command admissionをlive適用し、後続pane/windowのnew-session設定を保持する。AppleScript無効化はcached hierarchyを空にしてpending/new commandをexactly onceで拒否し、再有効化は生存standard objectを同じIDで再公開する。invalid→corrected reloadとresource identity/cleanupをM1 Developer JIT/Release AOTで受け入れ済み。global shortcut replacementはcandidate登録後に旧ownerを解放し、失敗時は旧登録を保持する |
