...
| QA-02 | xterm/Ghostty/Kitty black-box differential と real-app matrix。bug を最小 byte regression に還元 | P1 | 6/9 | `G:src/terminal/` tests and VT C examples | 266件のECMA-48/DEC/xterm/iTerm/Kitty/mintty selector・mode baselineと112件の製品実装照合、versioned observation/driver contract、Dart backend、bounded failure分類、3製品の固定adapter/config/provenanceと実PTY query captureを完了。編集/rendition/mode/queryのreview済み4-case Dart baselineは210 splitで固定し、DECRQSS実装後の12結果を一致8（Kitty semantic 1を含む）・所有者付きgap 0・unavailable 4へ分類。7 byte最小regressionはxterm exactとKitty raw差を通常gateで固定。vttest 2.7 (20251205) の34 familyを自動採用9・manual採用9・条件付き4・非採用12へ明文化。tmux/ssh/mosh/Neovim/Emacs/ncurses/fzf/lazygitを固定実行し、57,737 PTY byte・32 snapshot・48観測checkを証跡化。capture時526 reject/28 variant/16 gapを固定し、現行manifest replayではKitty keyboard controls、XTMODKEYS/XTQMODKEYS、application Escapeを実装後63 reject/6 variant/4 gapへ縮小。canonical key eventは別の実PTY product gateでquery reply・主/代替画面分離・release exact byteを両runtime固定し、Emacs/ncurses/Neovim/SSH一致4・documented-gap 4を通常gateで照合。Phase 6の9修正familyは390 input byte・417 whole/split/bytewise runのversion 1 corpusへ統合し、全owner、4件の明示的非対応gap、inventory、differential、97 byte/11 event parser traceを決定論的coverage reportと通常gateで照合 |
