...
| IN-09 | keybind/action registry、conflict、unbound/passthrough、menu shortcut arbitration | P0 | 5/7/8 | `G:src/input/Binding.zig`, `G:src/input/config.zig`, MenuShortcutManager | bounded immutable keybind engineと28個のsearchable application action catalog、repeatable typed設定、stable physical key/action ID、ordered override/unbound/passthrough、exactly-once application dispatch、native menu予約境界を通常製品へ接続。全key/action/default/reserved shortcutの生成freshness gateと、pane/application action、Settings、OSC 52 exact allow/deny、unbind、passthrough、invalid復旧、native menu優先、4 pane cleanupのM1両runtime受け入れを完了。semantic prompt previous/next、Command+矢印の方向別divider movement、Quick TerminalとSecure Keyboard Entryのtoggle、およびQuick Lookの標準Control-Command-D metadataは同じcatalog/menu/palette/keybind経路を使う。opt-in global shortcutだけは同じQuick Terminal toggle actionへexclusive system hot key eventを配送する |
...
| UI-09 | settings UI、command palette、terminal inspector | P1 | 8/10 | `G:macos/Sources/Features/Settings/`, `Command Palette/`, InspectorView | Shift-Command-Pのbounded searchable native command palette、shared `application.open-settings` action、Command-comma menuに加え、key入力前から全47 optionを表示するmodal root-document editorを実装。`/`検索、同一syntax表示のNORMAL/INSERT、commented optionの全行disabled色、modeをまたぐ全幅current-line背景、NORMAL/検索selectionのnative viewport追従、caret連動detail/rail、invalid draftの下線・非永続化、valid atomic save/shared reload、single-window/focus/全handle cleanupをM1 Developer JIT/Release AOTで受け入れ済み。Quick Terminal shortcutの登録状態・競合理由・旧登録保持、Secure Keyboard Entryのmode/ownership/自動取得/indicator設定、App Intentsのadmission/counter、notificationsのauthorization/failureをcontent-freeなbounded statusとして表示する。terminal/parser inspectorはPhase 10 |
...
| SEC-05 | crash/update/clipboard/shell integration の data flow と opt-in/out を privacy 文書化 | P0 | 10/11 | pinned crash/update/config behaviorを比較対象にする | Phase 1 local-run metadataのlocal-only/no-content policyを文書化。AppleScriptはterminal screen/history/process commandを公開せず、standard objectのstable ID/title/trusted local cwdだけをbounded cacheへ複製する。macOS Automation/TCCはsenderと利用者が所有し、製品はgrant/resetせず、`macos-applescript`でlive opt-outできる。外部permission/denial/recoveryは[reference](docs/reference/applescript.md)と[manual checklist](docs/phase10/applescript-manual-acceptance.md)へ記録。crash/update/clipboard/shell全体はPhase 10/11 |

## Compatibility、performance、distribution

| ID | parity unit / acceptance | 優先度 | Phase | pinned Ghostty evidence | 現在 |
| --- | --- | --- | --- | --- | --- |
| QA-01 | byte corpus、all chunk splits、property/fuzz、snapshot diagnostics | P0 | 3/6/9 | parser tests, `G:src/terminal/snapshot/`, `G:test/fuzz-libghostty/` | product parser全family・UTF-8 precedence、version 1 final-state/diagnostics、4件のsemantic corpusとshell/less/top/vim記録をall-split/bytewise検証。固定seed property、7件の境界別fuzz seed、112 mutationを完了。Phase 9の8 modern-protocol anchor、64 bit mutation、64 generated programはwhole/generated-chunk/bytewise/repeat/recoveryの680 execution・731,150 parse byteで同値性、復旧、bounded digestを検証し、OSC 133 bottom-scroll chunk差を最小回帰へ還元。opt-in bounded inspectorはprintable textをcountだけ、string payloadをlengthだけ保持し、全action/recovery familyのcanonical header evidence、eviction、observer failure、parser/inspector/export limitをdeterministic version 1 JSONへexport。97 byte/11 event fixtureをwhole・98 single split・bytewiseで通常freshness gate化 |
