...
| SCR-01 | primary/alternate screen、save/restore cursor、cursor shape/blink/visibility | P0 | 3 | `G:src/terminal/Screen.zig`, `G:src/terminal/ScreenSet.zig`, `G:src/terminal/cursor.zig` | bounded SoA visible screenとcursor position/save/restore/shape/blink/visibility stateを完了。primary/alternateは後続 |
| SCR-02 | wrap-pending、origin/insert/replace/autowrap/reverse-video mode | P0 | 3 | `G:src/terminal/Terminal.zig`, `G:src/terminal/modes.zig` | typed mode state、home/clamp、atomic non-cell resetを完了。print/editへの適用は次subtask |
| SCR-03 | top/bottom、left/right margin と scroll region | P0 | 3 | `G:src/terminal/Screen.zig`, `G:src/terminal/Terminal.zig` | validated vertical/horizontal margin stateとorigin/horizontal-mode境界を完了。scroll/editへの適用は次subtask |
