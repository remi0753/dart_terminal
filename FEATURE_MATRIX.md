...
| CAP-07 | XTGETTCAP、DECRQSS、window/size report | P1 | 6 | `G:src/terminal/dcs.zig`, `G:src/terminal/size_report.zig` | XTGETTCAPはbounded hex/list検証後、監査DBにないcapabilityを明示的 unavailable と応答し、`Ms`/OSC 52を先行広告しない。DECRQSSとwindow/size reportは後続task |
...
| IN-06 | precision/momentum scroll と terminal mouse report/local selection arbitration | P0 | 5 | `G:src/input/mouse.zig`, `G:src/input/mouse_encode.zig` | protocol v5 AppKit scroll、fractional/momentum accumulator、primary history、xterm wheel report、Shift override、alternate-screen cursor-keyを排他的にroutingし、M1両modeの実PTY/Metal受け入れを完了 |
...
| QA-02 | xterm/Ghostty/Kitty black-box differential と real-app matrix。bug を最小 byte regression に還元 | P1 | 6 | `G:src/terminal/` tests and VT C examples | 260件のECMA-48/DEC/xterm/iTerm selector・mode baselineと97件の製品実装照合、versioned observation/driver contract、Dart backend、bounded failure分類、3製品の固定adapter/config/provenanceと実PTY query captureを完了。編集/rendition/mode/queryのreview済み4-case Dart baselineは210 splitで固定し、12結果を一致6・所有者付きgap 2・unavailable 4へ分類。DECRQSS SGR差分を7 byteへ最小化し通常gateで固定。vttest 2.7 (20251205) の34 familyを自動採用9・manual採用9・条件付き4・非採用12へ明文化。tmux/ssh/mosh/Neovim/Emacs/ncurses/fzf/lazygitを固定実行し、57,737 PTY byte・32 snapshot・48観測checkを証跡化。capture時526 reject/28 variant/16 gapを固定し、現行manifest replayではcharacter-set/XTGETTCAP/title-stackを除く92 reject/20 variant/13 gapへ縮小。ncurses/SSH一致2・documented-gap 6を通常gateで固定 |
