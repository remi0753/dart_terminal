...
| CFG-06 | zsh/bash/fish/nushell integration、cwd/title/prompt mark/close hint | P1 | 8 | `G:src/shell-integration/`, `G:src/termio/shell_integration.zig` | 4 shellのdetect/forced/none分類、forced時を含むDarwin `/bin/bash`安全fallback、ZDOTDIR/ENV/XDG別のbounded transactional launch plan、version 1の5 resourceと固定size/SHA-256/UTF-8 contract、manifest/bundle audit、immutable new-pane projection、fake PTY全shell/fallbackと実zsh/Apple bash手動opt-in実行を完了。bundle zsh統合と明示none、user startup各1回、cleanup、2 session回収をM1 Developer JIT/Release AOT実製品で受け入れ済み。cwd/title/prompt mark/close hintは次のroadmap項目 |
