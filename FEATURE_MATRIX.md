...
| CAP-11 | Kitty graphics: transmit/storage/placement/z-index/scroll/erase/animation/eviction | P1 | 9 | `G:src/terminal/kitty/graphics*.zig`, `G:src/renderer/image.zig` | bounded APC grammar、process-worker direct RGB/RGBA/PNG+zlib decode、multipart/FIFO reply、主/代替画面別64 image・16 MiB storage、ID/number replacement、256 placement cap、static put/transmit-and-place/delete、通常negative/positive z、scrollback/margin clip/erase/reflow/alternate/RIS semantics、immutable viewport projection、bounded nearest-neighbor CPU oracleと共有color-atlas/Metal tile、1x/2x pixel golden、renderer再生成時の全量再公開を完了。animation frame transmit/edit/control/compose/deleteは同じworker FIFOを使い、imageごと64 total frame・画面ごと256 extra frame/16 MiB、generation-safe atomic state、exact replyで実装済み。visible imageだけをmonotonicに1 frameずつ進め、hidden/occluded/synchronized/recoveryでは通常schedulerの単一deadlineをpauseし、frame content generationでCPU/Metal投影・atlas invalidation/reuse・renderer再公開を1x/2x pixel検証済み。実zsh PTYから両runtimeでstatic query/multipart/history/erase/delete/render/resource回収を受け入れ済み。file/shared-memory transport、virtual/relative/extreme-negative z、deterministic resource evictionとanimationの実製品最終受け入れは後続 |
