...
| CFG-06 | zsh/bash/fish/nushell integration、cwd/title/prompt mark/close hint | P1 | 8 | `G:src/shell-integration/`, `G:src/termio/shell_integration.zig` | 4 shellのdetect/forced/none分類、Darwin `/bin/bash`安全fallback、ZDOTDIR/ENV/XDG別のbounded transactional launch plan、version 1の5 resourceと固定size/SHA-256/UTF-8 contract、manifest/bundle auditを完了。通常起動projectionとsemantic metadataは順次実装中 |
