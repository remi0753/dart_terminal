...
| IN-10 | Option-click cursor、semantic prompt selection、drag/drop、Services、Quick Look | P1/P2 | 5/10 | macOS Surface View, `G:macos/Sources/Features/Services/` | Quick Look向けのbounded word-at-cell/stale-generation contractと、Services/drop text、shell-quoted absolute file pathのtyped admissionを実装。native AppKit、runtime service declaration、製品routingはPhase 10の後続サブタスク |
...
| UI-05 | standard menu と Edit/Window/Shell/View action。terminal input と競合しない | P0 | 7 | `G:macos/Sources/App/MainMenu.xib`, action registry | 同一bounded catalogからApplication/File/Edit/Shell/View/Window menuを投影し、通常起動のimplemented handlerだけを動的にenable。menu/paletteのdispatch完了後にavailabilityを再同期し、Split Right/Down、Command+矢印divider movement、New Tab/Window、Close/Quitのexactly-once dispatch、terminal write 0、5 paneのkey/IME分離をDeveloper JIT/Release AOTで完了 |
