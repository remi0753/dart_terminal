...
| SCR-07 | primary/alternate resize と reflow。cursor、selection、wrapped-line identity を保つ | P0 | 3 | `G:src/terminal/PageList.zig`, `G:src/terminal/Screen.zig` | primary historyを含むatomic reflow、cursor/viewport、logical ID+reuse epoch+cell offset anchorを完了。selectionは後続 |
...
| SEC-01 | parser payload/count/value、scrollback、hyperlink、image、queue に hard cap | P0 | 3/5/9 | `G:src/terminal/Parser.zig`, kitty graphics storage, termio mailbox | product parser capとscrollbackのline/typed-byte/page capを完了。後続hyperlink/image/queue capは未実装 |
