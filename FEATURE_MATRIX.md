...
| IN-09 | keybind/action registry、conflict、unbound/passthrough、menu shortcut arbitration | P0 | 5/7/8 | `G:src/input/Binding.zig`, `G:src/input/config.zig`, MenuShortcutManager | bounded immutable keybind engineと26個のsearchable application action catalog、repeatable typed設定、stable physical key/action ID、ordered override/unbound/passthrough、exactly-once application dispatch、native menu予約境界を通常製品へ接続。全key/action/default/reserved shortcutの生成freshness gateと、pane/application action、Settings、OSC 52 exact allow/deny、unbind、passthrough、invalid復旧、native menu優先、4 pane cleanupのM1両runtime受け入れを完了。semantic prompt previous/next、Command+矢印の方向別divider movement、Quick Terminal toggleは同じcatalog/menu/palette/keybind経路を使う。opt-in global shortcutだけは同じtoggle actionへexclusive system hot key eventを配送する |
...
| UI-09 | settings UI、command palette、terminal inspector | P1 | 8/10 | `G:macos/Sources/Features/Settings/`, `Command Palette/`, InspectorView | Shift-Command-Pのbounded searchable native command palette、shared `application.open-settings` action、Command-comma menuに加え、key入力前から全42 optionを表示するmodal root-document editorを実装。`/`検索、同一syntax表示のNORMAL/INSERT、commented optionの全行disabled色、modeをまたぐ全幅current-line背景、NORMAL/検索selectionのnative viewport追従、caret連動detail/rail、invalid draftの下線・非永続化、valid atomic save/shared reload、single-window/focus/全handle cleanupをM1 Developer JIT/Release AOTで受け入れ済み。Quick Terminal shortcutの登録状態・競合理由・旧登録保持もbounded statusとして表示する。terminal/parser inspectorはPhase 10 |
...
| CFG-04 | reload と live/new-session/restart policy を option ごとに宣言 | P0 | 8 | `G:src/config/Config.zig`, macOS Config | 全42 optionにlive/new-session policyを宣言。明示Reload Configuration action、同一起動入力でのsingle-flight再解決、error候補（利用不能なfile値を含む）の全体拒否とlast-known-good保持、既存paneのkeybind/OptionとQuick Terminalのshortcut/screen/animation/autohideをlive適用、後続pane/windowのnew-session適用を実装し、invalid→corrected reloadとresource identity/cleanupをM1 Developer JIT/Release AOTで受け入れ済み。global shortcut replacementはcandidate登録後に旧ownerを解放し、失敗時は旧登録を保持する |
...
| CFG-07 | deprecated migration warning、effective config/action docs の schema generation | P1 | 8 | `G:src/config/`, CLI config commands | physical key、pane/application action、default binding、native予約shortcut、全42 optionのcanonical formatter、deprecated theme migration warning、`--help`/`--show-config`、configuration/CLI Markdownと、全option document/context detailをshared schema/catalogから生成して通常freshness gate化。ownerを作らないeffective-config early exit、invalid/unavailable startup diagnostic、Settingsのdraft validation/atomic save/no-invalid-reloadと利用不能値を含むcurrent/draft/application-policy表示をM1 Developer JIT/Release AOTで受け入れ済み。Quick Terminalの4 optionとtoggle actionも同じ生成境界へ含める |
